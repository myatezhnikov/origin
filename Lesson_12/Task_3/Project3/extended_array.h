...
public:
	...
	{
		int x = 0; 
		if (typeid(bool) == typeid(extended_array[0]))
		{
			for (const int i : extended_array) { if (i) x++;	}
			 return x;
		}
		else if (typeid(int) == typeid(extended_array[0]))
		{
			for (const int i : extended_array)
			{
				if (i != 0 && i != 1)
				{
					throw std::logic_error("Wrong numbers");
				}
				if (i == 1) x++;
			}
			return x;
